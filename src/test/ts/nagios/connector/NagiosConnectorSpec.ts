...
describe('NagiosConnector', function(): void {
    ...
    it('Test Methods', function(): void {
        expect(testling.getExpiry())
            .toBe(NagiosSpecDataProvider.EXPIRY);
        expect(testling.getApiUrl(monitor))
            .toBe('https://myhost:8080/myprefix/cgi-bin/status-json.cgi?host=all&callback=?');
        expect(testling.getHostInfoUrl(monitor.getHostname(), 'myid'))
            .toBe('https://myhost:8080/myprefix/cgi-bin/extinfo.cgi?type=1&host=myid');
    });
